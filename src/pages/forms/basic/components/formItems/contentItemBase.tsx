...
const ContentBase: React.FC<ContentBaseProps> = ({
  id,
  children,
  selectCahceId,
  onClick,
  ...
  const { moveItems, moveVirBox, copyItem, deleItem } = useContext(
    ContentContext,
  );
  ...
  return (
    <div
      className={isSelect ? 'content-view select-lable' : 'content-view'}
      onClick={() => onClick(id)}
      ...
      {isSelect && (
        <div className="select-operation">
          <Button
            ...
            shape="circle"
            icon={<CopyOutlined style={{ color: '#0DB3A6' }} />}
          />
          <Button
