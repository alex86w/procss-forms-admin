export default function(G6) {
  ...
  G6.registerBehavior('itemAlign', {
    ...
    _itemAlign(item) {
      ...
      const nodes = item._attrs.nodeId
        ? this.graph.getNodes().filter(n => n.get('id') !== item._attrs.nodeId)
        : this.graph.getNodes();
      each(nodes, node => {
        ...
        if (
          ...
          horizontalLines[0].dis < this.tolerance
        ) {
          item.attr({
        ...
        if (verticalLines.length > 0 && verticalLines[0].dis < this.tolerance) {
          ...
          p
            ...
            : (p = { verticals: [verticalLines[0]] });
          for (let i = 1; i < 3; i++)
            verticalLines[0].dis === verticalLines[i].dis &&
