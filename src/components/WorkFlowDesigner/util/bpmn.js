...
export function exportXML(json, canvas, createFile = true) {
  ...
  canvas.dataObjs.forEach(s => {
    dataObjs += `${tab(4)}<dataObject id="${s.id}" name="${
      s.name
    }" itemSubjectRef="xsd:${s.type}"></dataObject>\n`;
  });
  let signals = '';
  canvas.signalDefs.forEach(s => {
    signals += `${tab(2)}<signal id="${s.id}" name="${
      s.name
    }" flowable:scope="${s.scope}"></signal>\n`;
  });
  let messages = '';
  ...
  json.nodes.forEach(node => {
    BPMNShape +=
      `${tab(6)}<bpmndi:BPMNShape bpmnElement="${node.id}" id="BPMNShape_${
        node.id
      }">\n` +
      `${tab(8)}<omgdc:Bounds width="${node.size[0]}" height="${
        node.size[1]
      }" x="${node.x}" y="${node.y}"></omgdc:Bounds>\n` +
    ...
    switch (node.clazz) {
      ...
      case 'userTask': {
        ...
        processXML += `${tab(4)}<userTask id="${node.id}" name="${
          node.label
      ...
      case 'timerStart': {
        ...
        processXML += `${tab(4)}<startEvent id="${
          node.id
      ...
      case 'timerCatch': {
        ...
        processXML += `${tab(4)}<intermediateCatchEvent id="${
          node.id
  ...
  json.edges.forEach(edge => {
    BPMNEdge +=
      `${tab(6)}<bpmndi:BPMNEdge bpmnElement="${edge.source}_${
        edge.sourceAnchor
      }-${edge.target}_${edge.targetAnchor}" ` +
      `id="BPMNEdge_${edge.source}_${edge.sourceAnchor}-${edge.target}_${edge.targetAnchor}">\n` +
      `${tab(8)}<omgdi:waypoint x="${edge.startPoint.x}" y="${
        edge.startPoint.y
      }"></omgdi:waypoint>\n` +
      `${tab(8)}<omgdi:waypoint x="${edge.endPoint.x}" y="${
        edge.endPoint.y
      }"></omgdi:waypoint>\n` +
    ...
    if (edge.coditionExpression) {
      condition = `${tab(
        ...
        edge.coditionExpression
      }]]></conditionExpression>\n`;
    }
    processXML += `${tab(4)}<sequenceFlow id="${edge.source}_${
      edge.sourceAnchor
    }-${edge.target}_${edge.targetAnchor}" sourceRef="${
      edge.source
    }" targetRef="${edge.target}">${condition}</sequenceFlow>\n`;
  });
  processXML += `${tab(2)}</process>\n`;

  let BPMNDiagram =
    `${tab(2)}<bpmndi:BPMNDiagram id="BPMNDiagram_${id}">\n` +
    `${tab(
      4,
    )}<bpmndi:BPMNPlane bpmnElement="${id}" id="BPMNPlane_${id}">\n${BPMNShape}${BPMNEdge}${tab(
      4,
    )}</bpmndi:BPMNPlane>\n` +
